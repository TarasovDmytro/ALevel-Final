...
@PageTitle("Marks")
@Route(value = "student-views/marks/", layout = MainLayout.class)
@RolesAllowed("student")
public class StudentMarksView extends Div implements BeforeEnterObserver {
    private final SubjectService subjectService;
    private final MarkService markService;
    private final StudentService studentService;
    private Mark mark;

    private final String MARK_EDIT_ROUTE_TEMPLATE = "student-views/marks/%d/edit";
    ...
    private final TextField value = new TextField("Value of mark form 1 to 12");
    private final Button cancel = new Button("Cancel");
    private final Button save = new Button("Save");

    private final BeanValidationBinder<Mark> binder;
    private final Div editorLayoutDiv = new Div();

    public StudentMarksView(@Autowired MarkService markService, @Autowired SubjectService subjectService, @Autowired StudentService studentService) {
...
//        createEditorLayout(splitLayout);
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(StudentMarksView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (markId.isPresent()) {
            ...
            if (markFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(StudentMarksView.class);
            }
        }
    }
